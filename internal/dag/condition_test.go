...
func setupStore(t *testing.T) *doc.SQLiteStore {
	t.Helper()
...
func createNodes(t *testing.T, s *doc.SQLiteStore, ids ...string) {
	t.Helper()
	for _, id := range ids {
...
func createEdge(t *testing.T, s *doc.SQLiteStore, id, from, to string, typ model.EdgeType, cond *string, weight int) {
	t.Helper()
